...
public class DroneAgent : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        //set starting pos to -12,1.5,42
        Vector3 parentPosition = transform.parent.position;
        // choose random starting position between two possible starting positions
        if (Random.Range(0, 2) == 0)
        {
            transform.position = new Vector3(parentPosition.x -102.0f, 1.5f, parentPosition.z+219.0f); //for circular circuit
            //transform.position = new Vector3(parentPosition.x -11.3f, 1.5f, parentPosition.z+43.9f);
            //transform.position = new Vector3(parentPosition.x -42.9f, 1.5f, parentPosition.z-26.0f);
        }   
        else
        {
            //transform.position = new Vector3(parentPosition.x -11.3f, 1.5f, parentPosition.z+43.9f); 
